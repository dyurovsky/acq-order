...
cum.model <- function(order.num) {
  ...
  get.model <- function(select.measure,model) {
    filter(freqs.models,measure==select.measure)[[model]][[1]]
  }
  ...
  order.model.data <- freqs.data %>%
    ...
    slice(1:order.num) %>%
    group_by(measure) %>%
    mutate(predicted.freq = inv.logit(predict(get.model(unique(.$measure),"lm.freq"),
                                       newdata = filter(.,measure==unique(.$measure)))),
           predicted.nor = inv.logit(predict(get.model(unique(.$measure),"lm.nor"),
                                       newdata = filter(.,measure==unique(.$measure)))))
}
