...
order.data <- all.data %>%
  ...
  group_by(language, lexical_category,category, uni_lemma,item_id) %>%
...
freqs.data <- order.mean.data %>%
  ...
  group_by(measure) %>%
  mutate(freq.order = rank(-frequency)) %>%
  left_join(unilemma.order)

freqs.prod.data <- filter(freqs.data,measure=="produces")

model.norwegian <- lm(English ~ Norwegian,data=measure.data))$r.squared
