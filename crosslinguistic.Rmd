...
Get all data
```{r}
languages <- c("English", "Norwegian", "Croatian", "Spanish")
...
order.data <- all.data %>%
  ...
  group_by(language, lexical_category, uni_lemma,item_id) %>%
...
freqs.data <- order.mean.data %>%
 # select(-order,-mean.order,-lexical_category) %>%
  select(-prop,-mean.order,-lexical_category) %>%
...
#   do(lm.freq = glm(English ~ category * frequency, family="binomial", data=.),
#      lm.nor = glm(English ~ Norwegian, family="binomial", data=.))
...
#     predicted = inv.logit(predict(get.model(meas, model_type),
...
cors <- bind_rows(sapply(2:length(common.lemmas),cum.model,simplify = FALSE)) %>%
  gather(model,cor,freq,nor)

prod.data <- filter(freqs.data,measure=="understands") %>%
  mutate(predicted = predict(get.model("produces","lm.freq"))) %>%
  mutate(diff = (English - predicted))
  

ggplot(prod.data,aes(x=English,y=diff))+
