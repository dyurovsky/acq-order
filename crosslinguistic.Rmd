...
#     predicted = predict(get.model(meas, model_type),
#                               newdata = filtered_data)
    cor(filtered_data$English, predicted, method="spearman")
}
...
#              row_number() <= order.num + 10) %>%
    summarise(freq = get.prediction.lm.cor("lm.freq", unique(measure), .),
              nor = get.prediction.lm.cor("lm.nor", unique(measure), .)) %>%
    mutate(order.num = order.num)
}

cors <- bind_rows(sapply(seq(1, length(common.lemmas), 1),
                         cum.model, simplify = FALSE)) 
  

qplot(order.num, nor - freq, colour= measure, 
      geom = "point", 
      data=filter(cors, measure == "produces"))
