...
get.lang.lemmas <- function(lang) unique(filter(all.data, language == lang)$uni_lemma)
...
cum.model <- function(order.num) {
  ...
  nor.r2 <- function(measure.data) {summary(lm(English ~ Norwegian,
                                            data=measure.data))$r.squared} 
  
  freq.r2 <- function(measure.data) {summary(lm(English~freq.order*category,
                                             data=measure.data))$r.squared} 
  
  both.r2 <- function(measure.data) {
    summary(lm(English ~ Norwegian + freq.order*category, 
               data=measure.data))$r.squared} 
  
  tmp <- freqs.data %>%
    group_by(measure) %>%
    slice(1:order.num) %>%
    summarise(pred.freq = cor(predict(freqs.models$lm.freq[[1]],newdata = .),English))
    do(norwegian = nor.r2(.),
       freq = freq.r2(.),
       both = both.r2(.)) %>%
   # mutate(order.num=order.num)
    mutate(norwegian = norwegian[[1]],
           freq = freq[[1]],
           both = both[[1]],
           order.num = order.num)
}
...
r2s <- bind_rows(sapply(10:length(common.lemmas),cum.model,simplify = FALSE)) %>%
  gather(model,r2,norwegian:both)

r2.check <- filter(r2s,measure=="produces",model == "freq")


ggplot(r2s,aes(x = order.num,y=r2,color=model)) +
  facet_wrap(~ measure) + 
  geom_point()
